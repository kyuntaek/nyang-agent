 ...
 */
export function getAgentTimeContext(at: Date = new Date()): AgentTimeContext {
  const h = koreaHour(at);
  if (h >= 6 && h < 11) {
    return {
      slot: 'morning',
      slotLabelKr: '아침',
      questionTemplate: '{이름} 오늘 아침밥은요? 🍚',
  ...
  if (h >= 11 && h < 17) {
    return {
      slot: 'day',
      slotLabelKr: '낮',
      questionTemplate: '{이름} 오늘 간식 먹었어요? 🐟',
